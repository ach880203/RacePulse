...
→ **prompt-28** 으로 묶어서 수정 완료 ✅ 2026-05-19
...
- Phase 2 공식 완료 선언 조건: DESIGN Figma 업로드 ← **남은 유일한 조건**
- v2.md → v3.md 전환은 Phase 2 완료 공식 선언 후 진행
- 채팅창 용량 초과로 새 세션에서 이어서 진행

---

### [날짜: 2026-05-19] 11차 작업 세션 (prompt-27·28 구현)
- **참석**: 창현님
...
| 구분 | 작업 | 결과 |
|------|------|------|
| FE | MaintenancePage.tsx — 화요일 02:00~06:00 전체 화면 대체 + KST 카운트다운 | ✅ |
| FE | MaintenanceBanner.tsx — 월요일 14:00~23:59 사전 공지 + sessionStorage 닫기 | ✅ |
| FE | App.tsx — isMaintenanceTime() 조건부 렌더링 통합 | ✅ |
| BE | MaintenancePushScheduler.java — @Scheduled 월요일 22:00 전체 푸시 발송 | ✅ |
| BE | BackendApplication.java — @EnableScheduling 추가 | ✅ |
| BE | RaceController — GET /races/{id} + /entries + /result 엔드포인트 추가 | ✅ |
| BE | RaceResponse.java — trackType / prizeMoney / weather / raceClass 필드 추가 | ✅ |
| BE | RaceService.java — getRaceById / getRaceEntries / getRaceResult 구현 (JdbcTemplate) | ✅ |
| FE | axiosInstance.ts — 401 무한 대기 버그 수정 (failQueue 추가) | ✅ |
| FE | .env.development / .env.production — localhost 하드코딩 환경변수화 | ✅ |
| BE | DashboardService.java — Double null-safe 처리 (NPE 수정) | ✅ |
| BE | application-dev.yaml — Kakao / VAPID 민감 키 기본값 제거 | ✅ |

#### 빌드 검증
| 서버 | 결과 |
|------|------|
| FE `npm run build` | ✅ 성공 |
| BE `gradlew compileJava` | ✅ BUILD SUCCESSFUL |

#### 커밋 내역
```
feat: [prompt-27] 점검 모드 페이지 + 월요일 배너 + 푸시 알림 구현
fix: [prompt-28] 코드 리뷰 높음/중간 이슈 수정 6건
```

#### 잔여 이슈 (낮음 — 미수정)
- ML: pytest-asyncio 경고 (향후 버전 호환성, 기능 영향 없음)
- DB: V10 마이그레이션 중복 constraint — 이미 적용됐으므로 수정 시 checksum 충돌 위험. 신규 설치 시에만 문제 될 수 있음
